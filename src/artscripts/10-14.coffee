...
art.makeParticles = ->
  ...
  xposStart = @chance.integer({min: 5, max: 180})
  xpos = xposStart
  ypos = @chance.integer({min: 5, max: 100})
  particleW = @chance.integer({min: 42, max: 292})
  # particleH = @chance.integer({min: 42, max: 92})
  particleH = particleW

  @maxRadius = @chance.integer({min: 1, max: 24})
  ...
  while row < @count / 2
    ...
    while column < @count / 2
      @data.push {
        ...
        x: xpos
        y: ypos
        # angleStep: @chance.floating({ min: 0.001, max: 1}) + (i * 0.001)
        angleStep: @chance.floating({ min: 0.1, max: 0.5})
        targetx: xpos + @chance.integer { min: 25, max: @width}
        targety: ypos + @chance.integer { min: 25, max: @width}
        # xStepAmount: @chance.floating { min: 0.01, max: 5}
        # yStepAmount: @chance.floating { min: 0.01, max: 5}
        xStepAmount: @chance.floating { min: 0.01, max: 0.5}
        yStepAmount: @chance.floating { min: 0.01, max: 0.5}
        radius: @chance.integer({ min: 2, max: 6})
        sinRadius: @chance.integer { min: 10, max: @width / 4}
        # height: particleH / 4
        color: c.toString()
...
art.tick = ->
  ...
  @data.forEach((d,i) =>
    ...
    ###########################
    noiseValue = @simplex.noise2D(d.x, d.y) * @chance.floating {min: 0.05, max: 0.25}
    d.distance = pdistance(d.x,d.y,d.targetx,d.targety)

    d.angleStep += (noiseValue * 0.15)

    d.radius += (noiseValue * 0.25)
    d.radius = _.clamp(d.radius, 2, @maxRadius)
    ...
    xStepAmount = d.xStepAmount * @chance.floating {min: 0.1, max: 0.4}
    yStepAmount = d.xStepAmount * @chance.floating {min: 0.1, max: 0.4}



    # if noiseValue > 0
    # if d.x < d.targetx
    #   d.x += xStepAmount #+ (noiseValue * 0.25)
    # if d.x > d.targetx
    #   d.x -= xStepAmount #+ (noiseValue * 0.25)
    #
    # if d.y < d.targety
    #   d.y += yStepAmount #+ (noiseValue * 0.25)
    # if d.y > d.targety
    #   d.y -= yStepAmount #+ (noiseValue * 0.25)
    ...
    # c.s += @chance.floating({min: -0.5, max: 0.5})
