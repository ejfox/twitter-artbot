...
art.makeParticles = ->
  ...
  xposStart = @chance.integer({min: 40, max: @width})
  xpos = xposStart
  ypos = @chance.integer({min: 20, max: @width * 0.3})
  particleW = @chance.integer({min: 50, max: @width * 0.3})
  particleH = @chance.integer({min: 50, max: @width * 0.3})

  @maxRadius = @chance.integer({min: 10, max: @width * 0.5})
  ...
  while row < @count / 2
    ...
    while column < @count / 2
      @data.push {
        ...
        xStepAmount: @chance.floating { min: 0.1, max: 4}
        yStepAmount: @chance.floating { min: 0.1, max: 4}
        radius: @chance.integer { min: 0.5, max: 24}
        sinRadius: @chance.integer { min: 50, max: @width * 0.7}
        # height: particleH / 4
        color: c.toString()
        angle: @chance.integer { min: 0, max: 180}
        angleStep: @chance.floating { min: 0.1, max: 2}
      }
      i++
      xpos += particleW
      column++
...
art.tick = ->
  ...
  @data.forEach((d,i) =>
    ...
    # if @chance.bool()
    #   d.angleStep += @chance.floating { min: 0, max: 0.5}
    ...
    if @constrainEdges
      ...
      if d.y >= @height
        d.y = 0
        d.yStepAmount = @chance.floating { min: 0.1, max: 5}

      if d.x >= @width
        d.x = 0
        d.xStepAmount = @chance.floating { min: 0.1, max: 5}
