...
class GenArt
  constructor: (seed) ->
    ...
    # @opacity = 0.99
    @opacity = @chance.floating {min: 0.01, max: 0.5, fixed: 2}
    ...
    @multiColor = @chance.bool { chance: 2}
    ...
    @linkCount = @chance.integer({min: 1, max: @count / 2})
  ...
  makeParticles: =>
    ...
    @data = d3.range(@count).map (d,i) =>
...

      {
        ...
        # color: 'black'
        opacity: @opacity
        radius: @chance.integer {min: 1, max: 6}
      }
  ...
  makeSimulation: =>
    ...
    @simulation = d3.forceSimulation()
      ...
      .force 'links', d3.forceLink(@links).distance(=> @chance.integer {min: 10, max: @width})
