...
art.makeParticles = ->
  ...
  i = 0
  while row < @count / 2
    particleW = @chance.integer { min: 25, max: 120}#@chance.integer({min: 42, max: 192})
    particleH = @chance.integer { min: 25, max: 120}#@chance.integer({min: 42, max: 192})
    column = 0
    ...
    while column < @count / 2
      @data.push {
        ...
        x: xpos
        y: ypos
        angleStep: @chance.floating { min: 0.0001, max: 1.1}
        ...
        xStepAmount: @chance.floating { min: 0.001, max: 0.9}
        yStepAmount: @chance.floating { min: 0.001, max: 0.9}
...
art.tick = ->
  ...
  @data.forEach((d,i) =>
    ...
    d.angleStep += @chance.floating { min: -0.0001, max: 0.0001}
      ...
      if noiseValue < 0
        if @chance.bool()
          if @chance.bool {likelihood: 1}
            d.angle += 45
          if @chance.bool {likelihood: 50}
            d.angle += @chance.integer {min: 1, max: 45 }
          else if @chance.bool {likelihood: 1}
            d.angle -= 45
    ...
    if @doubles
      @ctx.arc d.y, d.x, d.radius/2, 0, 2*Math.PI
      @ctx.fillStyle = '#000'#d.color
      @ctx.fill()
