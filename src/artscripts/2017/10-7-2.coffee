...
art.makeParticles = ->
  ...
  particleW = @chance.integer({min: 42, max: 192})
  particleH = @chance.integer({min: 42, max: 192})
  ...
  while row < @count / 2
    ...
    while column < @count / 2
      @data.push {
        ...
        xStepAmount: @chance.floating { min: 0.01, max: 1}
        yStepAmount: @chance.floating { min: 0.01, max: 1}
