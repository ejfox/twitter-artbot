  ...
  art.makeParticles = function() {
    ...
    this.data = d3.range(this.count).map((function(_this) {
      return function(d, i) {
        ...
        if (i % 25 >= 1) {
          datum.x = _this.width / 2;
          datum.y = _this.width / 2;
        }
        return datum;
      };
    })(this));
    ...
    this.simulation = d3.forceSimulation(this.data).force('charge', d3.forceManyBody()).force('collide', d3.forceCollide(function(d) {
      return d.radius * 2;
    }).iterations(8)).force('link', d3.forceLink(this.links).distance(10).strength(0.9)).force('center', d3.forceCenter(this.width / 2, this.height / 2));
    return this.data;
  };
